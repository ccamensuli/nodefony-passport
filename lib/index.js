 ...
 *  Expose constructors.
 */
const nodefonyPassport = function (Factory) {

  const passportNodefony = class passportNodefony {
    constructor(factory) {
      ...
      return (context, next) => {
        ...
        if (this.callback) {
          if (!multi) {
            ...
            var challenges = this.failures.map(function (f) {
              return f.challenge;
            });
            var statuses = this.failures.map(function (f) {
              return f.status;
            });
    ...
    setStrategy(name, context, options, next) {
      ...
      this.factory.security.logger("STRATEGY  AUTHENTICATE  passport-" + name, "DEBUG");
  ...
  return function (_factory) {
    return {
      authenticate: function (passport, name, options, callback) {
        const inst = new passportNodefony(_factory);
